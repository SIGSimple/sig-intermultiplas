...
strQ = "SELECT *, IIf([flg_reajuste]=True,'Sim','Não') AS reajuste FROM tb_contrato_item WHERE id = " + Replace(rs_delete_us__MMColParam, "'", "''") + ""
...
<div align="center">
  <table border="0">
    ...
    <% While ((Repeat1__numRows <> 0) AND (NOT rs_delete_us.EOF)) %>
      <tr bgcolor="#CCCCCC">
        <td>
          <span class="style5"><%=(rs_delete_us.Fields.Item("cod_item").Value)%></span>
        </td>
        <td>
          <span class="style5"><%=(rs_delete_us.Fields.Item("dsc_item").Value)%></span>
        </td>
        <td>
          <span class="style5"><%=(rs_delete_us.Fields.Item("reajuste").Value)%></span>
        </td>
      </tr>
      <% 

...
%>
<%
Dim rs_inclui_pi__MMColParam
rs_inclui_pi__MMColParam = "1"
If (Request.QueryString("PI") <> "") Then 
  rs_inclui_pi__MMColParam = Request.QueryString("PI")
End If
%>
<%
Dim rs_inclui_pi
...
rs_inclui_pi.Source = "SELECT * FROM tb_pi WHERE PI = '" + Replace(rs_inclui_pi__MMColParam, "'", "''") + "'"
...
<body>
<p align="center"><strong><span class="style11">Altera&ccedil;&atilde;o de PIs</span></strong></p>
<form ACTION="<%=MM_editAction%>" METHOD="POST" name="form1">
  <table align="center">
    <tr valign="baseline">
      <td align="right" nowrap bgcolor="#CCCCCC" class="style9"><span class="style10">PI:</span></td>
      <td bgcolor="#CCCCCC"><input name="PI" type="text" class="style9" onblur="MM_validateForm('PI','','R');return document.MM_returnValue" value="<%=(rs_lista_pi.Fields.Item("PI").Value)%>" size="18">
...
%><option value="<%=(rs_predio.Fields.Item("cod_predio").Value)%>" <%If (Not isNull((rs_lista_pi.Fields.Item("cod_predio").Value))) Then If (CStr(rs_predio.Fields.Item("cod_predio").Value) = CStr((rs_lista_pi.Fields.Item("cod_predio").Value))) Then Response.Write("selected=""selected""") : Response.Write("")%> ><%=(rs_predio.Fields.Item("Expr1").Value)%></option>
          <%
  rs_predio.MoveNext()
Wend
...
%><option value="<%=(rs_construtora.Fields.Item("cod_construtora").Value)%>" <%If (Not isNull((rs_lista_pi.Fields.Item("cod_construtora").Value))) Then If (CStr(rs_construtora.Fields.Item("cod_construtora").Value) = CStr((rs_lista_pi.Fields.Item("cod_construtora").Value))) Then Response.Write("selected=""selected""") : Response.Write("")%> ><%=(rs_construtora.Fields.Item("Construtora").Value)%></option>
          <%
...
%><option value="<%=(rs_fiscal.Fields.Item("cod_fiscal").Value)%>" <%If (Not isNull((rs_lista_pi.Fields.Item("cod_fiscal").Value))) Then If (CStr(rs_fiscal.Fields.Item("cod_fiscal").Value) = CStr((rs_lista_pi.Fields.Item("cod_fiscal").Value))) Then Response.Write("selected=""selected""") : Response.Write("")%> ><%=(rs_fiscal.Fields.Item("Responsável").Value)%></option>
          <%
  rs_fiscal.MoveNext()
Wend
...
%>
  ...
  <input type="hidden" name="MM_recordId" value="<%= rs_inclui_pi.Fields.Item("PI").Value %>">
</form>
<p>&nbsp;</p>

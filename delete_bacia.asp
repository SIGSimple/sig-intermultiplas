...
If (Request.QueryString("idbacia") <> "") Then 
  rs_delete_us__MMColParam = Request.QueryString("idbacia")
End If
%>
<%
Dim rs_delete_us
...
rs_delete_us.Source = "SELECT * FROM tb_diretoria WHERE id = " + Replace(rs_delete_us__MMColParam, "'", "''") + ""
...
<div align="center">
  <table border="0">
    <tr bgcolor="#CCCCCC">
      <td><span class="style5">c&oacute;digo</span></td>
      <td><span class="style5">Nome da Bacia</span></td>
    </tr>
    <% While ((Repeat1__numRows <> 0) AND (NOT rs_delete_us.EOF)) %>
      <tr bgcolor="#CCCCCC">
        <td><span class="style3"><%=(rs_delete_us.Fields.Item("id").Value)%></span></td>
        <td><span class="style3"><%=(rs_delete_us.Fields.Item("desc_diretoria").Value)%></span></td>
      </tr>
      <% 

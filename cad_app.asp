...
<%
	...
	If Not IsEmpty(Request.Form) Then
		strQ = "SELECT * FROM tb_app Where 1 <> 1"
		...
		Set rs_update = Server.CreateObject("ADODB.Recordset")
			...
			rs_update("num_app") 			= Trim(Request.Form("num_app"))
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		<p align="center">
			<strong>
				<span class="style17">Autorizações p/ Intervenção em Apps</span>
			</strong>
		</p>
		...
		<form method="post" name="form1">
			...
			<table align="center">
				...
				<tr valign="baseline">
					<td align="right" nowrap bgcolor="#CCCCCC" class="style7">
						<span class="style22">Núm. Autorização:</span>
					</td>
					<td bgcolor="#CCCCCC">
						<input type="text" name="num_app" value="" size="10">
					</td>
				</tr>
				<tr valign="baseline">
					<td align="right" nowrap bgcolor="#CCCCCC" class="style7">
		...
		<div align="center">
			<table border="0">
				<tr bgcolor="#999999">
					...
					<td>&nbsp;</td>
					<td>
						<span class="style7">Núm. Autorização</span>
					</td>
					<td>
				...
				<%
					...
					strQ = "SELECT * from tb_app WHERE cod_empreendimento = '" & cod_empreendimento & "'"
				...
				<tr bgcolor="#CCCCCC">
					<td>
						<a href="altera_app.asp?id=<%=(rs_lista.Fields.Item("id").Value)%>&nme_municipio=<%=(Request.QueryString("nme_municipio"))%>&cod_empreendimento=<%=(Request.QueryString("cod_empreendimento"))%>&nme_empreendimento=<%=(Request.QueryString("nme_empreendimento"))%>">
							<img src="const/imagens/edit.gif" width="16" height="15" border="0" />
						</a>
					</td>
					<td>
						<a href="delete_app.asp?id=<%=(rs_lista.Fields.Item("id").Value)%>&nme_municipio=<%=(Request.QueryString("nme_municipio"))%>&cod_empreendimento=<%=(Request.QueryString("cod_empreendimento"))%>&nme_empreendimento=<%=(Request.QueryString("nme_empreendimento"))%>">
							<img src="const/imagens/delete.gif" width="16" height="15" border="0" />
						</a>
					</td>
					<td>
						<span class="style5"><%=(rs_lista.Fields.Item("num_app").Value)%></span>
					</td>
					<td>
					...
					<td>
						<form id="form-upload" method="post" enctype="multipart/form-data"
							action="novo_upload.asp?id=<%=(rs_lista.Fields.Item("id").Value)%>&folder=APP&retUrl=<%=(Request.ServerVariables("URL"))%>?<%=(Request.QueryString)%>">
						...
						<%
							...
							strF = "SELECT * FROM tb_app_arquivo WHERE cod_referencia = " & cod_convenio
						...
						%>
							<ul>
								<li>
									<a href="download.asp?path=/ARQUIVOS/APP&filename=<%=(rs_lista.Fields.Item("id").Value)%>_<%=(rs_files.Fields.Item("nme_arquivo").Value)%>">

...
if (CStr(Request("MM_delete")) = "form1" And CStr(Request("MM_recordId")) <> "") Then
  ...
  MM_editTable = "tb_pi_contrato"
  MM_editColumn = "id"
  MM_recordId = "" + Request.Form("MM_recordId") + ""
  MM_editRedirectUrl = "cad_contrato_empreendimento.asp"
...
strQ = "SELECT tb_pi_contrato.id, c_lista_dados_obras.Código, c_lista_dados_obras.PI, c_lista_dados_obras.nome_empreendimento, c_lista_contrato.num_autos, c_lista_contrato.num_contrato FROM (tb_pi_contrato INNER JOIN c_lista_dados_obras ON tb_pi_contrato.cod_empreendimento = c_lista_dados_obras.Código) INNER JOIN c_lista_contrato ON tb_pi_contrato.cod_contrato = c_lista_contrato.id WHERE tb_pi_contrato.id = " + Replace(rs_delete_us__MMColParam, "'", "''") + ""
...
<div align="center">
  <table border="0">
    <tr bgcolor="#CCCCCC">
      <td>
        <span class="style5">Empreendimento</span>
      </td>
      <td>
        <span class="style5">Contrato</span>
      </td>
    </tr>
    <% While ((Repeat1__numRows <> 0) AND (NOT rs_delete_us.EOF)) %>
      <tr bgcolor="#CCCCCC">
        <td>
          <span class="style3"><%=(rs_delete_us.Fields.Item("PI").Value)%> - <%=(rs_delete_us.Fields.Item("nome_empreendimento").Value)%></span>
        </td>
        <td>
          <span class="style3"><%=(rs_delete_us.Fields.Item("num_autos").Value)%> - <%=(rs_delete_us.Fields.Item("num_contrato").Value)%></span>
        </td>
      </tr>
      <% 

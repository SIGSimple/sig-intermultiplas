...
<SELECT name=campo_busca class=texto_pagina>
  ...
  <OPTION value="cod_depto" <% If Trim(Request("campo_busca")) = Trim("cod_depto") Then : Response.Write "selected" : End If %>>cod_programa</OPTION>
  <OPTION value="desc_depto" <% If Trim(Request("campo_busca")) = Trim("desc_depto") Then : Response.Write "selected" : End If %>>desc_programa</OPTION>
...
%>
  ...
  <TD width="12%" valign=top nowrap style="cursor: hand" onClick="window.open('<%=Request.ServerVariables("SCRIPT_NAME")%>?Ordem=cod_depto+<%=Ordem%>', '_self')"><%If Left(Request.QueryString("Ordem"), 9) = "cod_depto" Then : Response.Write "<img src=""imagens/ordem_" & Ordem & ".gif"" width=9 height=10>&nbsp;" : End If%><b>cod_programa</b></TD>
  <TD width="63%" valign=top nowrap style="cursor: hand" onClick="window.open('<%=Request.ServerVariables("SCRIPT_NAME")%>?Ordem=desc_depto+<%=Ordem%>', '_self')"><%If Left(Request.QueryString("Ordem"), 10) = "desc_depto" Then : Response.Write "<img src=""imagens/ordem_" & Ordem & ".gif"" width=9 height=10>&nbsp;" : End If%><b>desc_programa</b></TD>

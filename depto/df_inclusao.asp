...
If Not IsEmpty(Request.Form) Then
  ...
  If campo_duplicado = false Then
    ...
    objRS.CursorType = 0
    objRS.LockType = 3
    strQ = "SELECT * FROM tb_depto Where 1 <> 1"
    objRS.Open strQ, objCon, , , &H0001
    objRS.Addnew()
    If objRS.Fields("cod_depto").properties("IsAutoIncrement") = False Then
      objRS("cod_depto") = Trim(Request.Form("cod_depto"))
    End If
    If objRS.Fields("desc_depto").properties("IsAutoIncrement") = False Then
      objRS("desc_depto") = Trim(Request.Form("desc_depto"))
    End If
    If objRS.Fields("sigla").properties("IsAutoIncrement") = False Then
      objRS("sigla") = Trim(Request.Form("sigla"))
    End If
    objRS.Update
    objRS.Close
    Set objRS = Nothing
...
<TABLE border=0 cellpadding=2 cellspacing=1 class=tabela_formulario>
  <TR class=titulo_campos><TD>cod_programa<br>
<%If objRS.Fields("cod_depto").properties("IsAutoIncrement") = False Then%>
<INPUT style="width=350" type="text" name="cod_depto" maxlength="255" value="" onKeyPress="desabilita_cor(this)" class=campos_formulario>
<%
Else
...
%>
  ...
  <TR class=titulo_campos><TD>desc_programa<br>
<%If objRS.Fields("desc_depto").properties("IsAutoIncrement") = False Then%>
<INPUT style="width=350" type="text" name="desc_depto" maxlength="255" value="" onKeyPress="desabilita_cor(this)" class=campos_formulario>
<%
Else
  Response.Write "<B>(Automático)</B>"
End If
%>
  ...
  <TR class=titulo_campos><TD>sigla<br>
<%If objRS.Fields("sigla").properties("IsAutoIncrement") = False Then%>
<INPUT style="width=350" type="text" name="sigla" maxlength="255" value="" onKeyPress="desabilita_cor(this)" class=campos_formulario>
<%
Else
  Response.Write "<B>(Automático)</B>"
End If

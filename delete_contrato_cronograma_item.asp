...
if (CStr(Request("MM_delete")) = "form1" And CStr(Request("MM_recordId")) <> "") Then
  ...
  MM_editTable = "tb_contrato_cronograma_item"
  MM_editColumn = "id"
  MM_recordId = "" + Request.Form("MM_recordId") + ""
  MM_editRedirectUrl = "cad_contrato_cronograma_item.asp"
...
strQ = "SELECT * FROM c_lista_itens_planejamento WHERE id = " + Replace(rs_delete_us__MMColParam, "'", "''") + ""
    ...
    <% While ((Repeat1__numRows <> 0) AND (NOT rs_delete_us.EOF)) %>
      <tr bgcolor="#CCCCCC">
        ...
        <td>
          <span class="style5">
            <%
              mes = CaptalizeText(MonthName(rs_delete_us.Fields.Item("mes_planejamento").Value,True))
              ano = rs_delete_us.Fields.Item("ano_planejamento").Value
              Response.Write mes & "/" & ano
            %>
          </span>
        </td>
        <td align="right">
          <span class="style5 vlr"><%=(rs_delete_us.Fields.Item("vlr_planejamento").Value)%></span>
        </td>
      </tr>
      <% 

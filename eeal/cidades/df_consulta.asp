...
objCon.Open MM_cpf_STRING
  ...
  If Request.Form("recordno") <> "" Then
    ...
    If indice <> "" Then strQ_delete = " SELECT * FROM cMunicipios WHERE " & indice
...
If Trim(Request("string_busca")) <> "" Then
  ...
  Else
    strQ = strQ & " Where 1 <> 1"
    strQ = strQ & " Or cod_mun LIKE '%" & Trim(Request("string_busca")) & "%'"
    strQ = strQ & " Or Municipios LIKE '%" & Trim(Request("string_busca")) & "%'"
  End If
End If
...
strQ_indice = "SELECT * FROM cMunicipios WHERE 1 <> 1"
...
<SELECT name=campo_busca class=texto_pagina>
  ...
  <OPTION value="cod_mun" <% If Trim(Request("campo_busca")) = Trim("cod_mun") Then : Response.Write "selected" : End If %>>cod_mun</OPTION>
  <OPTION value="Municipios" <% If Trim(Request("campo_busca")) = Trim("Municipios") Then : Response.Write "selected" : End If %>>Municipios</OPTION>
</SELECT>
...
%>
  ...
  <TD width="10%" valign=top nowrap style="cursor: hand" onClick="window.open('<%=Request.ServerVariables("SCRIPT_NAME")%>?Ordem=cod_mun+<%=Ordem%>', '_self')"><%If Left(Request.QueryString("Ordem"), 7) = "cod_mun" Then : Response.Write "<img src=""imagens\ordem_" & Ordem & ".gif"" width=9 height=10>&nbsp;" : End If%><b>cod_mun</b></TD>
  <TD width="90%" valign=top nowrap style="cursor: hand" onClick="window.open('<%=Request.ServerVariables("SCRIPT_NAME")%>?Ordem=Municipios+<%=Ordem%>', '_self')"><%If Left(Request.QueryString("Ordem"), 10) = "Municipios" Then : Response.Write "<img src=""imagens\ordem_" & Ordem & ".gif"" width=9 height=10>&nbsp;" : End If%><b>Municipios</b></TD>
  </TR>
...
%>
    ...
    <TD><%=(objRS.Fields.Item("cod_mun").Value)%></TD>

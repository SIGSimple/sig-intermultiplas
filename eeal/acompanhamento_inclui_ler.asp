...
If (CStr(Request("MM_update")) = "form2" And CStr(Request("MM_recordId")) <> "") Then
  ...
  MM_columnsStr = "PI|',none,''|[Descrição da Intervenção FDE]|',none,''|cod_construtora|none,none,NULL|[Número do Contrato]|',none,''|[Dígito do Contrato]|none,none,NULL|Órgão|',none,''|cod_fiscal|none,none,NULL|[Gerenciadora Mede ?]|none,1,0|cod_situacao|none,none,NULL|[Data da Abertura]|',none,NULL|[Data do TRP]|',none,NULL|[Data do TRD]|',none,NULL"
...
rs_pi.Source = "SELECT tb_pi.PI, [tb_predio].[cod_predio] & ' - ' & [tb_predio].[Nome_Unidade] AS Expr1, c_Semaforico1.[Avanço Físico Atual],  [tb_predio].[cod_predio]  FROM c_Semaforico1 INNER JOIN (tb_predio INNER JOIN tb_pi ON tb_predio.cod_predio = tb_pi.cod_predio) ON c_Semaforico1.[PI-item] = tb_pi.PI  WHERE PI = '" + Replace(rs_pi__MMColParam, "'", "''") + "'  ORDER BY tb_pi.PI;"
rs_pi.CursorType = 0
...
Recordset4.Source = "SELECT tb_pi.PI, tb_predio.Município  FROM tb_predio INNER JOIN tb_pi ON tb_predio.cod_predio = tb_pi.cod_predio  WHERE PI = '" + Replace(Recordset4__MMColParam, "'", "''") + "'  ORDER BY tb_predio.Município"
...
<div align="center">
  ...
  <table width="823" border="0">
    <tr bgcolor="#FFFFFF">
      <td bgcolor="#FFFFFF" class="style27"><a href="med_constr_inclui_ler.asp?pi=<%=(rs_pi.Fields.Item("PI").Value)%>" target="_blank">Medições</a></td>
    </tr>
  </table>
...
%>
      ...
      <tr valign="baseline" bgcolor="#999999">
        ...
        <td colspan="4" bgcolor="#EBEBEB"><div align="left"><strong><span class="style28">Descri&ccedil;&atilde;o da Interven&ccedil;&atilde;o FDE</span></strong>
                <textarea name="Descrio_da_Interveno_FDE" cols="50" class="style28"><%=(rs_atualiza_pi.Fields.Item("Descrição da Intervenção FDE").Value)%></textarea>
        </div></td>
      </tr>
      <tr valign="baseline" bgcolor="#999999">
    ...
    <tr valign="baseline">
      ...
      <td bordercolor="#CCCCCC" bgcolor="#CCCCCC"><label>
        ...
        <div id="LOEmpresaA" style="display:none;">
          Nº LO:<input name="LO" type="text"/>ex: XX00000
        </div>      </td>
    </tr>

...
<%
	...
	If Not IsEmpty(Request.Form) Then
		strQ = "SELECT * FROM tb_outorga Where 1 <> 1"
		...
		Set rs_update = Server.CreateObject("ADODB.Recordset")
			...
			rs_update("num_outorga") 		= Trim(Request.Form("num_outorga"))
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		<p align="center">
			<strong>
				<span class="style17">Cadastro de Outorgas</span>
			</strong>
		</p>
		...
		<form method="post" name="form1">
			...
			<table align="center">
				...
				<tr valign="baseline">
					...
					<td bgcolor="#CCCCCC">
						<input type="text" name="num_outorga" value="" size="10">
					</td>
				</tr>
				<tr valign="baseline">
					<td align="right" nowrap bgcolor="#CCCCCC" class="style7">
		...
		<div align="center">
			<table border="0">
				<tr bgcolor="#999999">
					...
					<td>&nbsp;</td>
					<td>
						<span class="style7">Núm. Outorga</span>
					</td>
					<td>
				...
				<%
					...
					strQ = "SELECT * from tb_outorga WHERE cod_empreendimento = " & cod_empreendimento
				...
				<tr bgcolor="#CCCCCC">
					<td>
						<a href="altera_outorga.asp?id=<%=(rs_lista.Fields.Item("id").Value)%>&nme_municipio=<%=(Request.QueryString("nme_municipio"))%>&cod_empreendimento=<%=(Request.QueryString("cod_empreendimento"))%>&nme_empreendimento=<%=(Request.QueryString("nme_empreendimento"))%>">
							<img src="const/imagens/edit.gif" width="16" height="15" border="0" />
						</a>
					</td>
					<td>
						<a href="delete_outorga.asp?id=<%=(rs_lista.Fields.Item("id").Value)%>&nme_municipio=<%=(Request.QueryString("nme_municipio"))%>&cod_empreendimento=<%=(Request.QueryString("cod_empreendimento"))%>&nme_empreendimento=<%=(Request.QueryString("nme_empreendimento"))%>">
							<img src="const/imagens/delete.gif" width="16" height="15" border="0" />
						</a>
					</td>
					<td>
						<span class="style5"><%=(rs_lista.Fields.Item("num_outorga"))%></span>
					</td>
					<td>
					...
					<td>
						<form id="form-upload" method="post" enctype="multipart/form-data"
							action="novo_upload.asp?id=<%=(rs_lista.Fields.Item("id").Value)%>&folder=OUTORGA&retUrl=<%=(Request.ServerVariables("URL"))%>?<%=(Request.QueryString)%>">
						...
						<%
							...
							strF = "SELECT * FROM tb_outorga_arquivo WHERE cod_referencia = " & cod_convenio
						...
						%>
							<ul>
								<li>
									<a href="download.asp?path=/ARQUIVOS/OUTORGA&filename=<%=(rs_lista.Fields.Item("id").Value)%>_<%=(rs_files.Fields.Item("nme_arquivo").Value)%>">

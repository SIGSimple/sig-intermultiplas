...
If (CStr(Request("MM_update")) = "form1" And CStr(Request("MM_recordId")) <> "") Then
  ...
  MM_editTable = "tb_licenca_ambiental"
  MM_editColumn = "id"
  MM_recordId = "" + Request.Form("MM_recordId") + ""
  MM_editRedirectUrl = "cad_licenca.asp"
  MM_fieldsStr  = "num_licenca|value|cod_tipo_licenca|value|dta_concessao|value|dta_vencimento|value|cod_agencia_liberacao|value|dsc_observacoes|value"
  MM_columnsStr = "num_licenca|',none,''|cod_tipo_licenca|none,none,NULL|dta_concessao|',none,NULL|dta_vencimento|',none,NULL|cod_agencia_liberacao|none,none,NULL|dsc_observacoes|',none,''"
...
rs.Source = "SELECT * FROM tb_licenca_ambiental WHERE id = " + Replace(rs__MMColParam, "'", "''")
rs.CursorType = 0
rs.CursorLocation = 2
rs.LockType = 1
rs.Open()
...
<body>
<p align="center"><strong><span class="style17">Alteração de Licença Ambiental </span></strong></p>
    <form method="post" action="<%=MM_editAction%>" name="form1">
      ...
      <table align="center">
        ...
        <tr valign="baseline">
          ...
          <td bgcolor="#CCCCCC">
            <input type="text" name="num_licenca" value="<%=(rs.Fields.Item("num_licenca").Value)%>" size="10">
          </td>
        </tr>
        <tr valign="baseline">
          <td align="right" nowrap bgcolor="#CCCCCC" class="style7">
                ...
                If Not rs_combo.EOF Then
                  While Not rs_combo.EOF
                    If Trim(rs_combo.Fields.Item("dsc_tipo_licenca").Value) <> "" Then
                      ...
                      If Lcase(rs_combo.Fields.Item("id").Value) = Lcase(rs.Fields.Item("cod_tipo_licenca").Value) then
                        Response.Write "selected"
                      End If
                      Response.Write ">" & (rs_combo.Fields.Item("dsc_tipo_licenca").Value) & "</OPTION>"
                    End If
                    rs_combo.MoveNext
                  Wend
                End If
                ...
                If Not rs_combo.EOF Then
                  While Not rs_combo.EOF
                    If Trim(rs_combo.Fields.Item("nme_agencia").Value) <> "" Then
                      Response.Write "      <OPTION value='" & (rs_combo.Fields.Item("id").Value) & "'"
                      If Lcase(rs_combo.Fields.Item("id").Value) = Lcase(rs.Fields.Item("cod_agencia_liberacao").Value) then
                        Response.Write "selected"
                      End If
                      Response.Write ">" & (rs_combo.Fields.Item("nme_agencia").Value) & "</OPTION>"
                    End If
                    rs_combo.MoveNext
                  Wend
                End If

...
rs_filtro_combo_cmd.CommandText = "SELECT tb_pi.*, tb_situacao_pi.desc_situacao AS situacao_interna, tb_situacao_pi_1.desc_situacao AS situacao_externa FROM (tb_situacao_pi RIGHT JOIN tb_pi ON tb_situacao_pi.cod_situacao = tb_pi.cod_situacao) LEFT JOIN tb_situacao_pi AS tb_situacao_pi_1 ON tb_pi.cod_situacao_externa = tb_situacao_pi_1.cod_situacao WHERE cod_predio = ?" 
  ...
  <table border="0">
    ...
    <% While ((Repeat2__numRows <> 0) AND (NOT rs_filtro_combo.EOF)) %>
      <tr bgcolor="#CCCCCC">
        ...
        <td class="style15"><%=(rs_filtro_combo.Fields.Item("situacao_interna").Value)%></td>
        <td class="style15"><%=(rs_filtro_combo.Fields.Item("situacao_externa").Value)%></td>
      </tr>
      <% 
